...
async function seed() {
    ...
    // Reset autoincrement counters so IDs always start at 1
    await AppDataSource.query("DELETE FROM sqlite_sequence WHERE name = 'concert'");
    await AppDataSource.query("DELETE FROM sqlite_sequence WHERE name = 'ticket'");
